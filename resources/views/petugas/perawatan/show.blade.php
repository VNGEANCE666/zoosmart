            ...
            <form method="POST" action="{{ route('petugas.perawatan.store') }}">
                ...
                <div class="row">
                    <div class="form-group col-6">
                        ...
                        <select name="id_petugas" class="form-control" required disabled>
                            @foreach ($petugas as $p)
                                <option value="{{ $p->id }}" <?php $p->id == $perawatan->id_petugas ? "{{selected disabled}}" : null ?> >{{ $p->nama_petugas }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group col-6">
                        <label for="id_hewan">Nama Hewan</label>
                        <select name="id_hewan" class="form-control" required disabled>
                            @foreach ($hewans as $hewan)
                                <option value="{{ $hewan->id }}" <?php $hewan->id == $perawatan->id_petugas ? "{{selected disabled}}" : null ?>>{{ $hewan->nama_hewan }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-6">
                        ...
                        <input type="text" class="form-control" name="jenis_perawatan" id="jenis_perawatan" value="{{$perawatan->jenis_perawatan}}"
                            placeholder="Jenis Perawatan" required disabled>
                    </div>
                    <div class="form-group col-6">
                        ...
                        <input type="datetime-local" class="form-control" name="jadwal_perawatan" id="jadwal_perawatan" value="{{$perawatan->jadwal_perawatan}}"
                            placeholder="Jadwal Perawatan" required disabled>
                    </div>
                </div>

                <a class="btn btn-secondary" href="{{route('petugas.perawatan.index')}}">Kembali</a>
            </form>
        </div>
    </div>
@endsection
